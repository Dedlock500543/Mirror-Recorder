...
public class GuiMirrorHelp extends GuiScreen{
    ...
    private void layout(){
        ...
        listTop=winY+headH+22;footerY=winY+winH-25;
    ...
    @Override public void initGui(){
        ...
        lang=add(new StyledButton(LANG,winX+winW-14-bw*2-6,footerY,bw,bh,Lang.name()).accent(UiTheme.PURPLE).tip(Lang.tip("tip.lang")));
        back=add(new StyledButton(BACK,winX+winW-14-bw,footerY,bw,bh,Lang.t("common.back")).primary().accent(UiTheme.ACCENT).tip(Lang.tip("tip.back")));
        rewrap();
    }
    private void rewrap(){
    ...
    @Override public void drawScreen(int mx,int my,float pt){
        ...
        UiTheme.chip(fontRenderer,counter,winX+winW-14-cw,winY+(headH-11)/2,UiTheme.PURPLE);
    ...
    private static final String[][] RU={
        ...
        {"Внешний вид",
         ...
         "§7/mirror stop — остановить запись или повтор."},
        {"Защита и корзина",
         "§fСтоп при уроне",
         ...
         "§8Хранятся 20 последних удалений, слот для возврата должен быть пустым."}
    };
    ...
    private static final String[][] EN={
        ...
        {"Look and feel",
         ...
         "§7/mirror stop — stop recording or playback."},
        {"Safety and trash",
         "§fStop when hurt",
         ...
         "§8The last 20 deletions are kept and the target slot has to be empty."}
    };
    ...
    private static final String[][] DE={
        ...
        {"Sicherheit und Papierkorb", "§fStopp bei Schaden", "§7Wenn dich etwas trifft, stoppt die Mod sofort Aufnahme oder Wiedergabe und lässt alle Tasten los.", "§8Schalter: Einstellungen → Steuerung → «Stopp bei Schaden».", "", "§fPapierkorb", "§7Eine gelöschte Aufnahme verschwindet nicht: eine Kopie landet im Papierkorb.", "§7Einstellungen → Oberfläche → «Papierkorb»: tippe eine Karte an, dann den großen grünen Knopf «Wiederherstellen in Slot».", "§7«Letzte Löschung» holt zurück, was du gerade entfernt hast. «Alles leeren» fragt noch einmal.", "§8Die letzten 20 Löschungen werden aufbewahrt, und der Zielslot muss leer sein."},
    };

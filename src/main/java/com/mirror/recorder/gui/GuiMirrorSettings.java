...
public class GuiMirrorSettings extends GuiScreen{
    ...
    private static final int K_REC_MOVE=1,K_REC_JUMP=2,K_REC_SNEAK=3,K_REC_SPRINT=4,K_REC_CLICK=5,K_REC_CHAT=6,
        ...
        K_PATH=44,K_STYLE=45,K_WIDTH=46,K_LENGTH=47,K_PSTEP=48,K_PSIZE=49,K_HEIGHT=50,K_FADE=51,K_LWALLS=52,K_RESET_LOOK=53,K_COLOR_TARGET=54,K_SPEED=55,K_SOUNDS=56,K_GUARD_DMG=57,K_TRASH=58,K_SPEED_DEF=59,K_LIMIT=60,K_AUTO_LEAVE=61,K_LEAVE_CYCLES=62;
    private static final int T_TOGGLE=0,T_STEP=1,T_CYCLE=2,T_ACTION=3,T_INFO=4,T_HEAD=5,HEAD_KEY_BASE=-2000,GROUP_HEAD_H=19,GROUP_HEAD_STEP=23;
    ...
    private static int colorTarget=0,visElement=0;private int tab=0,scroll=0,visPrevL,visPrevT,visPrevR,visPrevB;private boolean colorPanelOpen=false;private int[] rowY=new int[0];
    ...
    public GuiMirrorSettings(RecorderManager m,RecorderConfig c,GuiScreen p,int startTab){manager=m;config=c;parent=p;tab=Math.max(0,Math.min(TAB_COUNT-1,startTab));snapshot();}
    ...
    private int[] ints(){return new int[]{config.getStartDelay(),config.getPathLineR(),config.getPathLineG(),config.getPathLineB(),
        ...
        config.getMarkerOwnAlpha(),config.getMarkerLabelDistance(),config.getLoopLimit(),config.getAutoLeaveCycles()};}
    private void applyInts(int[] v){config.setStartDelay(v[0]);config.setPathLineR(v[1]);config.setPathLineG(v[2]);config.setPathLineB(v[3]);
        ...
        config.setMarkerAlpha(v[13]);config.setMarkerLabelDistance(v[14]);if(v.length>15)config.setLoopLimit(v[15]);if(v.length>16)config.setAutoLeaveCycles(v[16]);}
    ...
    private boolean[] bools(){return new boolean[]{config.isStopOnMove(),config.isApplyRotation(),config.isRouteStabilization(),
        ...
        config.isRecordMovement(),config.isRecordJump(),config.isRecordSneak(),config.isRecordSprint(),config.isRecordInteraction(),config.isRecordChat(),
        ...
        config.isMarkerLabelUnlimited(),config.isMarkerThroughWalls(),config.isSoundsEnabled(),config.isGuardOnDamage(),config.isAutoLeaveEnabled()};}
    private void applyBools(boolean[] v){config.setStopOnMove(v[0]);config.setApplyRotation(v[1]);config.setRouteStabilization(v[2]);
        ...
        config.setRecordMovement(v[8]);config.setRecordJump(v[9]);config.setRecordSneak(v[10]);config.setRecordSprint(v[11]);config.setRecordInteraction(v[12]);config.setRecordChat(v[13]);
        config.setPlaybackMovement(v[14]);config.setPlaybackJump(v[15]);config.setPlaybackSneak(v[16]);config.setPlaybackSprint(v[17]);config.setPlaybackInteraction(v[18]);config.setPlaybackChat(v[19]);
        config.setPathLineFade(v[20]);config.setPathLineThroughWalls(v[21]);config.setMarkerCustomColor(v[22]);config.setMarkerPulse(v[23]);config.setMarkerLabel(v[24]);
        config.setMarkerLabelUnlimited(v[25]);config.setMarkerThroughWalls(v[26]);config.setSoundsEnabled(v[27]);config.setGuardOnDamage(v[28]);if(v.length>29)config.setAutoLeaveEnabled(v[29]);}
    ...
    // ------------------------------------------------ категории
    private String tabName(int i){switch(i){case 0:return s("Воспроизведение","Playback","Відтворення","Wiedergabe","Odtwarzanie");case 1:return s("Управление","Control","Керування","Steuerung","Sterowanie");case 2:return s("Визуализация","Visualization","Візуалізація","Visualisierung","Wizualizacja");case 3:return s("Интерфейс","Interface","Інтерфейс","Oberfläche","Interfejs");default:return s("Справка","Help","Довідка","Hilfe","Pomoc");}}
    private String tabHint(int i){switch(i){case 0:return s("Повтор и запуск","Replay and start","Повтор і запуск","Wiedergabe und Start","Odtwarzanie i start");case 1:return s("Возврат и стабилизация","Return and stabilization","Повернення і стабілізація","Rückkehr und Stabilisierung","Powrót i stabilizacja");case 2:return s("Что рисуется в мире","What is drawn in the world","Що малюється у світі","Was in der Welt gezeichnet wird","Co jest rysowane w świecie");case 3:return s("Подсказки и язык","Tooltips and language","Підказки і мова","Tooltips und Sprache","Podpowiedzi i język");default:return s("Как работает мод","How the mod works","Як працює мод","Wie die Mod funktioniert","Jak działa mod");}}
    private String tabIcon(int i){switch(i){case 0:return "\u25b6";case 1:return "\u2699";case 2:return "\u2248";case 3:return "\u2726";default:return "?";}}
    private int tabColor(int i){switch(i){case 0:return UiTheme.BLUE;case 1:return UiTheme.GREEN;case 2:return UiTheme.PURPLE;case 3:return UiTheme.ACCENT;default:return UiTheme.BLUE;}}
    ...
    private void buildRows(){
        ...
        switch(tab){
            case 0:
                head(s("ЧТО ПОВТОРЯТЬ","WHAT TO REPLAY","ЩО ПОВТОРЮВАТИ","WAS WIEDERHOLEN","CO POWTARZAĆ"));
                ...
                head(s("ЧТО ЗАПИСЫВАТЬ","WHAT TO RECORD","ЩО ЗАПИСУВАТИ","WAS AUFNEHMEN","CO NAGRYWAĆ"));
                add(T_TOGGLE,K_REC_MOVE,s("Записывать движение","Record movement","Записувати рух","Bewegung aufnehmen","Nagrywaj ruch"),s("Шаги W A S D","W A S D steps","Кроки W A S D","W A S D-Schritte","Kroki W A S D"),"\u25b6",c,
                    tt("§fЗаписывать движение\n§7Вкл: ваши шаги попадают в запись.\n§7Выкл: шаги не сохраняются — повтор будет стоять на месте.\n§8Влияет только на новые записи.","§fRecord movement\n§7On: your steps are stored.\n§7Off: steps are not stored, so the replay stands still.\n§8Applies to new recordings only.","§fЗаписувати рух\n§7Увімк: ваші кроки потрапляють у запис.\n§7Вимк: кроки не зберігаються — повтор стоятиме на місці.\n§8Впливає лише на нові записи.","§fBewegung aufnehmen\n§7An: deine Schritte werden gespeichert.\n§7Aus: Schritte werden nicht gespeichert — die Wiedergabe steht still.\n§8Gilt nur für neue Aufnahmen.","§fNagrywaj ruch\n§7Wł: twoje kroki trafiają do nagrania.\n§7Wył: kroki nie są zapisywane — powtórka stoi w miejscu.\n§8Dotyczy tylko nowych nagrań."));
                add(T_TOGGLE,K_REC_JUMP,s("Записывать прыжки","Record jumps","Записувати стрибки","Sprünge aufnehmen","Nagrywaj skoki"),s("Пробел","Space","Пробіл","Leertaste","Spacja"),"\u2191",c,
                    tt("§fЗаписывать прыжки\n§7Вкл: пробел попадает в запись.\n§7Выкл: прыжки не сохраняются.\n§8Влияет только на новые записи.","§fRecord jumps\n§7On: the space bar is stored.\n§7Off: jumps are not stored.\n§8Applies to new recordings only.","§fЗаписувати стрибки\n§7Увімк: пробіл потрапляє у запис.\n§7Вимк: стрибки не зберігаються.\n§8Впливає лише на нові записи.","§fSprünge aufnehmen\n§7An: die Leertaste wird gespeichert.\n§7Aus: Sprünge werden nicht gespeichert.\n§8Gilt nur für neue Aufnahmen.","§fNagrywaj skoki\n§7Wł: spacja trafia do nagrania.\n§7Wył: skoki nie są zapisywane.\n§8Dotyczy tylko nowych nagrań."));
                add(T_TOGGLE,K_REC_SNEAK,s("Записывать приседание","Record sneaking","Записувати присідання","Schleichen aufnehmen","Nagrywaj kucanie"),s("Shift","Shift","Shift","Shift","Shift"),"\u2193",c,
                    tt("§fЗаписывать приседание\n§7Вкл: Shift попадает в запись.\n§7Выкл: приседания не сохраняются.\n§8Влияет только на новые записи.","§fRecord sneaking\n§7On: Shift is stored.\n§7Off: sneaking is not stored.\n§8Applies to new recordings only.","§fЗаписувати присідання\n§7Увімк: Shift потрапляє у запис.\n§7Вимк: присідання не зберігаються.\n§8Впливає лише на нові записи.","§fSchleichen aufnehmen\n§7An: Shift wird gespeichert.\n§7Aus: Schleichen wird nicht gespeichert.\n§8Gilt nur für neue Aufnahmen.","§fNagrywaj kucanie\n§7Wł: Shift trafia do nagrania.\n§7Wył: kucanie nie jest zapisywane.\n§8Dotyczy tylko nowych nagrań."));
                add(T_TOGGLE,K_REC_SPRINT,s("Записывать бег","Record sprinting","Записувати біг","Sprinten aufnehmen","Nagrywaj sprint"),s("Спринт","Sprint","Спринт","Sprint","Sprint"),"\u00bb",c,
                    tt("§fЗаписывать бег\n§7Вкл: спринт попадает в запись.\n§7Выкл: бег не сохраняется.\n§8Влияет только на новые записи.","§fRecord sprinting\n§7On: sprinting is stored.\n§7Off: sprinting is not stored.\n§8Applies to new recordings only.","§fЗаписувати біг\n§7Увімк: спринт потрапляє у запис.\n§7Вимк: біг не зберігається.\n§8Впливає лише на нові записи.","§fSprinten aufnehmen\n§7An: Sprinten wird gespeichert.\n§7Aus: Sprinten wird nicht gespeichert.\n§8Gilt nur für neue Aufnahmen.","§fNagrywaj sprint\n§7Wł: sprint trafia do nagrania.\n§7Wył: sprint nie jest zapisywany.\n§8Dotyczy tylko nowych nagrań."));
                add(T_TOGGLE,K_REC_CLICK,s("Записывать клики и предметы","Record clicks and items","Записувати кліки й предмети","Klicks und Gegenstände aufnehmen","Nagrywaj kliknięcia i przedmioty"),s("Удары, еда, блоки, хотбар","Hits, food, blocks, hotbar","Удари, їжа, блоки, хотбар","Schläge, Essen, Blöcke, Hotbar","Uderzenia, jedzenie, bloki, hotbar"),"\u25cf",c,
                    tt("§fЗаписывать клики и предметы\n§7Вкл: кнопки мыши, выбор слота и инвентарь попадают в запись.\n§7Выкл: запишется только перемещение.\n§8Влияет только на новые записи.","§fRecord clicks and items\n§7On: mouse buttons, slot changes and inventory are stored.\n§7Off: only movement is stored.\n§8Applies to new recordings only.","§fЗаписувати кліки й предмети\n§7Увімк: кнопки миші, вибір слота й інвентар потрапляють у запис.\n§7Вимк: запишеться тільки переміщення.\n§8Впливає лише на нові записи.","§fKlicks und Gegenstände aufnehmen\n§7An: Maustasten, Slot-Wechsel und Inventar werden gespeichert.\n§7Aus: nur Bewegung wird gespeichert.\n§8Gilt nur für neue Aufnahmen.","§fNagrywaj kliknięcia i przedmioty\n§7Wł: przyciski myszy, zmiana slotu i ekwipunek trafiają do nagrania.\n§7Wył: zapisany zostanie tylko ruch.\n§8Dotyczy tylko nowych nagrań."));
                add(T_TOGGLE,K_REC_CHAT,s("Записывать чат","Record chat","Записувати чат","Chat aufnehmen","Nagrywaj czat"),s("Сообщения и команды","Messages and commands","Повідомлення й команди","Nachrichten und Befehle","Wiadomości i komendy"),"\u270e",c,
                    tt("§fЗаписывать чат\n§7Вкл: отправленные сообщения и команды попадают в запись.\n§7Выкл: чат не сохраняется.\n§8Пароли и личные сообщения лучше не записывать.","§fRecord chat\n§7On: messages and commands you send are stored.\n§7Off: chat is not stored.\n§8Better not to record passwords or private messages.","§fЗаписувати чат\n§7Увімк: надіслані повідомлення й команди потрапляють у запис.\n§7Вимк: чат не зберігається.\n§8Паролі й особисті повідомлення краще не записувати.","§fChat aufnehmen\n§7An: gesendete Nachrichten und Befehle werden gespeichert.\n§7Aus: Chat wird nicht gespeichert.\n§8Passwörter und private Nachrichten besser nicht aufnehmen.","§fNagrywaj czat\n§7Wł: wysłane wiadomości i komendy trafiają do nagrania.\n§7Wył: czat nie jest zapisywany.\n§8Haseł i prywatnych wiadomości lepiej nie nagrywać."));
                head(s("ЗАПУСК И ЦИКЛ","START AND LOOP","ЗАПУСК І ЦИКЛ","START UND SCHLEIFE","START I PĘTLA"));
                ...
                add(T_TOGGLE,K_STOP,s("Остановить при моём движении","Stop on my movement","Зупинити при моєму русі","Bei eigener Bewegung stoppen","Zatrzymaj przy moim ruchu"),s("Нажатие W A S D прерывает повтор","W A S D aborts the playback","Натискання W A S D перериває повтор","W A S D bricht die Wiedergabe ab","Wciśnięcie W A S D przerywa powtórkę"),"\u25a0",c,
                    tt("\u00a7fОстановка, если вы пошли сами\n\u00a77Вкл: нажали W, A, S или D — повтор сразу прекращается.\n\u00a77Удобно, когда нужно быстро вмешаться.\n\u00a78Обычно: вкл.","\u00a7fStop when you move\n\u00a77On: pressing W, A, S or D stops the playback at once.\n\u00a77Handy when you need to step in quickly.\n\u00a78Usually: on.","§fЗупинка, якщо ви пішли самі\n§7Увімк: натиснули W, A, S або D — повтор одразу припиняється.\n§7Зручно, коли треба швидко втрутитися.\n§8Зазвичай: увімк.","§fStopp, wenn du dich selbst bewegst\n§7An: W, A, S oder D gedrückt — die Wiedergabe stoppt sofort.\n§7Praktisch, wenn du schnell eingreifen willst.\n§8Meistens: an.","§fZatrzymanie, gdy ruszysz sam\n§7Wł: wciśnięcie W, A, S lub D natychmiast przerywa powtórkę.\n§7Wygodne, gdy trzeba szybko zareagować.\n§8Zwykle: wł."));
                ...
                break;
            case 1:
                head(s("ПОВЕДЕНИЕ ЦИКЛА","LOOP BEHAVIOR","ПОВЕДІНКА ЦИКЛУ","SCHLEIFENVERHALTEN","ZACHOWANIE PĘTLI"));
                ...
                head(s("АВТОВЫХОД С СЕРВЕРА","AUTO LEAVE","АВТОВИХІД ІЗ СЕРВЕРА","SERVER AUTOMATISCH VERLASSEN","AUTOWYJŚCIE Z SERWERA"));
                ...
                head(s("ВОЗВРАТ И ТОЧНОСТЬ","RETURN AND ACCURACY","ПОВЕРНЕННЯ І ТОЧНІСТЬ","RÜCKKEHR UND GENAUIGKEIT","POWRÓT I DOKŁADNOŚĆ"));
                ...
                head(s("БЕЗОПАСНОСТЬ","SAFETY","БЕЗПЕКА","SICHERHEIT","BEZPIECZEŃSTWO"));
            ...
            case 2:
                if(visElement==0){
                    head(s("ОСНОВНЫЕ ЭЛЕМЕНТЫ","BASIC ELEMENTS","ОСНОВНІ ЕЛЕМЕНТИ","GRUNDELEMENTE","ELEMENTY PODSTAWOWE"));
                    ...
                    head(s("ЦВЕТ И СТИЛЬ","COLOR AND STYLE","КОЛІР І СТИЛЬ","FARBE UND STIL","KOLOR I STYL"));
                    ...
                    head(s("ПОЛОЖЕНИЕ","POSITION","ПОЛОЖЕННЯ","POSITION","POZYCJA"));
                    ...
                    head(s("РАЗМЕРЫ","SIZE","РОЗМІРИ","GRÖSSEN","ROZMIARY"));
                    ...
                    head(s("ЭФФЕКТЫ","EFFECTS","ЕФЕКТИ","EFFEKTE","EFEKTY"));
            ...
            case 3:
                ...
                head(s("ДАННЫЕ И СБРОС","DATA AND RESET","ДАНІ І СКИДАННЯ","DATEN UND RESET","DANE I RESET"));
    ...
    private boolean stateOf(int key){
        switch(key){
            case K_REC_MOVE:return config.isRecordMovement();
            case K_REC_JUMP:return config.isRecordJump();
            case K_REC_SNEAK:return config.isRecordSneak();
            case K_REC_SPRINT:return config.isRecordSprint();
            case K_REC_CLICK:return config.isRecordInteraction();
            case K_REC_CHAT:return config.isRecordChat();
    ...
    private void toggleKey(int key){
        ...
        switch(key){
            case K_REC_MOVE:config.setRecordMovement(v);break;
            case K_REC_JUMP:config.setRecordJump(v);break;
            case K_REC_SNEAK:config.setRecordSneak(v);break;
            case K_REC_SPRINT:config.setRecordSprint(v);break;
            case K_REC_CLICK:config.setRecordInteraction(v);break;
            case K_REC_CHAT:config.setRecordChat(v);break;
    ...
    private int lastVisibleRow(int from,int startY){
        ...
        for(int i=from;i<rows.size();i++){
            Row r=rows.get(i);
            if(r.type==T_HEAD){if(count>0)cursor+=GROUP_GAP-2;if(cursor+GROUP_HEAD_H>listBottom-4)break;count++;last=i;cursor+=GROUP_HEAD_STEP;continue;}
            if(cursor+rowH>listBottom-4)break;
            count++;last=i;cursor+=rowH+5;
        }
        return last;
    }
    ...
    @Override public void initGui(){
        ...
        contentX=sideX+sideW+14;contentRight=winX+winW-12;contentTop=winY+headH+26;contentBottom=footerY-10;
        sideRowH=Math.max(16,Math.min(28,(contentBottom-sideTop-(TAB_COUNT-1)*4)/TAB_COUNT));
        for(int i=0;i<TAB_COUNT;i++){
            int y=sideTop+i*(sideRowH+4);
        ...
        if(tab==2){
            ...
            visPrevB=visPrevT+ph;int chipY=ph>0?visPrevB+4:visPrevT,chipH=20,gap=6,chipW=Math.max(88,(contentRight-contentX-18-gap)/2);
        ...
        for(int i=scroll;i<rows.size();i++){
            Row r=rows.get(i);
            if(r.type==T_HEAD){
                if(placed>0)cursorY+=GROUP_GAP-2;
            ...
            int y=cursorY;rowY[i]=y;placed++;cursorY+=rowH+5;
            ...
            if(r.type==T_TOGGLE)add(new StyledButton(ACT_BASE+r.key,contentX+6,y,w,rowH,r.label).asToggle(stateOf(r.key)).icon(r.icon).sub(r.note).tip(r.tip));
                ...
                int bs=stepBtn(),by=y+(rowH-bs)/2,bx=stepBlockX();
                add(new StyledButton(DEC_BASE+r.key,bx,by,bs,bs,"\u2212").accent(UiTheme.GRAY).compact().flat()
                ...
                add(new StyledButton(INC_BASE+r.key,bx+bs+stepValW(),by,bs,bs,"+").accent(UiTheme.GRAY).compact().flat()
                ...
                if(r.key==K_SPEED){
                    int dx=bx+stepBlockW()+8,dw=Math.min(86,contentRight-14-dx);
        ...
        cancel=add(new StyledButton(CANCEL,winX+winW-14-bw*2-7,footerY,bw,bh,Lang.t("common.cancel")).accent(UiTheme.GRAY)
        ...
        save=add(new StyledButton(SAVE,winX+winW-14-bw,footerY,bw,bh,Lang.t("common.save")).primary().accent(UiTheme.GREEN)
    ...
    private void valueRow(Row r,int y,int mx,int my){
        ...
        int size=Math.min(rowH-8,14),tx=l+8,bs=stepBtn(),vx=stepBlockX();
        ...
        UiTheme.drawFittedShadow(fontRenderer,r.label,tx,blockY,lim,UiTheme.TEXT);
        ...
        if(r.type==T_STEP){
            ...
            UiTheme.drawCenteredFitted(fontRenderer,valueOf(r.key),vx+bs+stepValW()/2f,by+(bs-fontRenderer.FONT_HEIGHT)/2,stepValW()-4,UiTheme.TEXT,false);
        }
    }
    @Override public void drawScreen(int mx,int my,float pt){
        ...
        if(changed()){String chip=s("ИЗМЕНЕНО","CHANGED","ЗМІНЕНО","GEÄNDERT","ZMIENIONO");int cw=fontRenderer.getStringWidth(chip)+8;UiTheme.chip(fontRenderer,chip,winX+winW-14-cw,winY+13,UiTheme.AMBER);}
        ...
        int sideBottom=sideTop+(TAB_COUNT-1)*(sideRowH+4)+sideRowH;
    ...
    @Override protected void keyTyped(char c,int key)throws IOException{
        ...
        if(key==Keyboard.KEY_UP||key==Keyboard.KEY_LEFT){tab=(tab+TAB_COUNT-1)%TAB_COUNT;scroll=0;initGui();return;}
        if(key==Keyboard.KEY_DOWN||key==Keyboard.KEY_RIGHT){tab=(tab+1)%TAB_COUNT;scroll=0;initGui();return;}

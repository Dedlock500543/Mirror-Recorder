...
public class GuiMirrorSlotTools extends GuiScreen{
    ...
    @Override public void initGui(){
        ...
        headH=Math.min(34,Math.max(26,wh/9));padX=16;innerW=ww-padX*2;
        int x=wx+padX,half=(innerW-10)/2;
        int backH=19,backY=wy+wh-10-backH,bh=32,delH=22,avail=wh-headH-10;
        int fixed=18+bh*2+8+10+delH+10+backH+10;
        if(avail-fixed<46){bh=24;delH=20;fixed=18+bh*2+8+10+delH+10+backH+10;}
        if(avail-fixed<46){bh=20;delH=16;fixed=18+bh*2+8+10+delH+10+backH+10;}
        pathH=avail-fixed-50>=46?40:0;
        infoY=wy+headH+10;infoH=Math.max(28,avail-fixed-(pathH>0?pathH+10:0));
        int gridY=infoY+infoH+18;
        ...
        dup=add(new StyledButton(DUP,x+half+10,gridY,half,bh,s("Дублировать","Duplicate","Дублювати","Duplizieren","Duplikuj")).accent(UiTheme.PURPLE).icon("copy")
        ...
        imp=add(new StyledButton(IMPORT,x+half+10,row2,half,bh,s("Импорт","Import","Імпорт","Import","Import")).accent(UiTheme.GREEN).icon("\u2193")
    ...
    @Override public void drawScreen(int mx,int my,float pt){
        ...
        UiTheme.sectionLabel(fontRenderer,s("ДЕЙСТВИЯ","ACTIONS","ДІЇ","AKTIONEN","AKCJE"),l,infoY+infoH+9,innerW);
        if(pathH>0){

...
public class StyledButton extends GuiButton{
    ...
    @Override public void drawButton(Minecraft mc,int mx,int my,float pt){
        ...
        int left=x+UiTheme.BTN_PAD;
        if(icon!=null&&width>=44&&height>=16){
            int size=UiTheme.ICON,pad=UiTheme.BTN_PAD;
            UiTheme.badge(mc.fontRenderer,icon,x+pad,y+(height-size)/2+shift,size,acc,enabled&&(on||!toggle));
            left=x+pad+size+UiTheme.ICON_GAP;
        }
        int right=x+width-UiTheme.BTN_PAD;
        if(toggle){
            int sw=width<64?Math.max(14,width/3):UiTheme.SWITCH_W,sh=Math.min(UiTheme.SWITCH_H,Math.max(9,height-8)),sx=x+width-sw-UiTheme.BTN_PAD,sy=y+(height-sh)/2+shift;
            UiTheme.switchAt(sx,sy,sw,sh,knob,enabled);
            right=sx-UiTheme.GAP;
        }else if(value!=null){
            String val=value;int maxVal=Math.max(16,width/2-8),vw=UiTheme.fittedWidth(mc.fontRenderer,val,maxVal);
            UiTheme.drawFitted(mc.fontRenderer,val,x+width-UiTheme.BTN_PAD-vw,y+(height-mc.fontRenderer.FONT_HEIGHT)/2+shift,maxVal,enabled?UiTheme.lerp(UiTheme.TEXT_DIM,UiTheme.TEXT,h):UiTheme.TEXT_MUTE);
            right=x+width-UiTheme.BTN_PAD-vw-UiTheme.GAP;
        }

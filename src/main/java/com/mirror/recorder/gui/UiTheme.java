...
public final class UiTheme{
    ...
    public static int pillAt(FontRenderer fr,String text,int x,int y,int fill,int brdr,int tc){
        int w=fr.getStringWidth(text)+PILL_PAD*2;fillRound(x,y,x+w,y+PILL_H,6f,tint(fill,0xFFFFFF,0.08f),fill);
        strokeRound(x,y,x+w,y+PILL_H,6f,brdr,1f);fr.drawString(text,x+PILL_PAD,pillTextY(fr,y),tc);return w;
    }
    ...
    public static void sectionHead(FontRenderer fr,String text,int x,int y,int w,int accent){
        ...
        drawFitted(fr,"\u00a7l"+text,x+11,y,Math.max(16,w-11),TEXT_DIM);
    }
    ...
    public static void rowCard(int l,int t,int r,int b,float hover,boolean selected,int accent,float fade){
        ...
        if(selected&&b-t>9){
            float bar=0.92f*fa;
            fillRound(l+2,t+3,l+4,b-3,1f,alpha(CARD_SELB,bar),alpha(CARD_SELB,bar));
        }
    ...
    /** One pill geometry for chips and pills: same height, padding and optical centering everywhere. */
    public static final int PILL_H=12,PILL_PAD=5;
    /** Единая метрическая сетка: отступы, интервалы и размеры повторяющихся элементов. */
    public static final int PAD=12,GAP=6,BTN_H=20,ROW_H=26,ROW_GAP=5,BTN_PAD=8,SWITCH_W=22,SWITCH_H=11,ICON=13,ICON_GAP=5,NAV_W=22;
    private static int pillTextY(FontRenderer fr,int y){return y+Math.max(0,(PILL_H-fr.FONT_HEIGHT+1)/2);}
    public static int chip(FontRenderer fr,String text,int x,int y,int color){
        int w=fr.getStringWidth(text)+PILL_PAD*2;fillRound(x,y,x+w,y+PILL_H,6f,alpha(color,0.16f),alpha(color,0.16f));
        strokeRound(x,y,x+w,y+PILL_H,6f,alpha(color,0.5f),1f);fr.drawString(text,x+PILL_PAD,pillTextY(fr,y),color);return w;
    }
    ...
    public static final int TABLE_HEAD=14,TABLE_ROW=12;
    ...
    /** Колонка значений встаёт по самой широкой подписи, а не по доле ширины:
     *  иначе короткие подписи висят далеко от своих значений. */
    public static int tableValueX(FontRenderer fr,String[] labels,int l,int r){
        if(fr==null)return tableValueX(l,r);
        int need=fr.getStringWidth(Lang.s("Параметр","Parameter","Параметр","Parameter","Parametr"));
        if(labels!=null)for(String s:labels)if(s!=null)need=Math.max(need,fr.getStringWidth(s));
        return Math.max(l+40,Math.min(l+need+GAP*2,Math.max(l+40,r-60)));
    }
    ...
    public static int infoTable(FontRenderer fr,String[] labels,String[] values,int l,int t,int r,int bottom){
        ...
        int vx=tableValueX(fr,labels,l,r),labelW=Math.max(24,vx-l-GAP),valueW=Math.max(24,r-vx);
        drawFitted(fr,Lang.s("Параметр","Parameter","Параметр","Parameter","Parametr"),l,t,labelW,0xFF6F8AA8);
        ...
        for(int i=0;i<n;i++){
            ...
            drawFitted(fr,labels[i]==null?"":labels[i],l,y,labelW,TEXT_DIM);

...
@SideOnly(Side.CLIENT) public class PathLineRenderer{
    ...
    @SubscribeEvent public void onRenderWorldLast(RenderWorldLastEvent e){
        ...
        float pt=e.getPartialTicks();double cx=p.lastTickPosX+(p.posX-p.lastTickPosX)*pt,cy=p.lastTickPosY+(p.posY-p.lastTickPosY)*pt,cz=p.lastTickPosZ+(p.posZ-p.lastTickPosZ)*pt;int cycle=manager.getPlaybackCycle();float r=config.getPathLineR()/255f,g=config.getPathLineG()/255f,b=config.getPathLineB()/255f,a=config.getPathLineAlpha()/255f;
        ...
        if(style!=2){
            additive();GL11.glLineWidth(width+3.6f);strokePath(buf,tess,count,cycle,cx,cy,cz,lift,r,g,b,a*0.16f*breath,fade,travel,0.10f);
            normalBlend();GL11.glLineWidth(width+1.4f);strokePath(buf,tess,count,cycle,cx,cy,cz,lift,r,g,b,a*0.28f*breath,fade,travel,0.16f);
            GL11.glLineWidth(width);strokePath(buf,tess,count,cycle,cx,cy,cz,lift,Math.min(1f,r+0.18f),Math.min(1f,g+0.18f),Math.min(1f,b+0.18f),a*breath,fade,travel,0.42f);
        }
        if(style!=0){
            additive();GL11.glLineWidth(Math.max(2.2f,width+1.2f));strokeMarks(buf,tess,count,step,cycle,cx,cy,cz,lift,ps*1.55d,r,g,b,a*0.20f*breath,fade,travel);
            normalBlend();GL11.glLineWidth(Math.max(1.2f,width));strokeMarks(buf,tess,count,step,cycle,cx,cy,cz,lift,ps,r,g,b,a*breath,fade,travel);
        }
    ...
    private void strokePath(BufferBuilder buf,Tessellator tess,int count,int cycle,double cx,double cy,double cz,double lift,float r,float g,float b,float a,boolean fade,float travel,float boost){
        ...
        for(int i=0;i<count;i++){
            ...
            buf.pos(trajectory.expectedX(f,cycle)-cx,trajectory.expectedY(f,cycle)-cy+lift,trajectory.expectedZ(f,cycle)-cz).color(r,g,b,av).endVertex();
        }
        tess.draw();
    }
    private void strokeMarks(BufferBuilder buf,Tessellator tess,int count,int step,int cycle,double cx,double cy,double cz,double lift,double ps,float r,float g,float b,float a,boolean fade,float travel){
        buf.begin(GL11.GL_LINES,DefaultVertexFormats.POSITION_COLOR);
        for(int i=0;i<count;i+=step){
            Frame f=cachedFrames[i];double px=trajectory.expectedX(f,cycle)-cx,py=trajectory.expectedY(f,cycle)-cy+lift,pz=trajectory.expectedZ(f,cycle)-cz;float av=shade(a,i,count,fade,travel,0.35f);

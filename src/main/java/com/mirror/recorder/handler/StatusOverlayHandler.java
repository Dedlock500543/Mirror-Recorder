...
public class StatusOverlayHandler{
    ...
    @SubscribeEvent public void onTick(TickEvent.ClientTickEvent e){
        ...
        else if(manager.isPlaying()){int frames=Math.max(1,manager.getSlotFrameCount(slot)),current=Math.min(frames,manager.getCurrentFrameIndex()+1);if(manager.isLooping()){int limit=manager.getPlaybackLoopLimit();title=s("Цикл ","Loop ","Цикл ","Schleife ","Pętla ")+manager.getPlaybackCycle()+"/"+(limit==0?s("без лимита","unlimited","без ліміту","unbegrenzt","bez limitu"):String.valueOf(limit));color=UiTheme.PURPLE;}else{title=s("Воспроизведение","Playback","Відтворення","Wiedergabe","Odtwarzanie");color=UiTheme.BLUE;}detail=s("Слот ","Slot ","Слот ","Slot ","Slot ")+slot+"  ·  "+s("Кадр ","Frame ","Кадр ","Frame ","Klatka ")+current+"/"+frames;progress=current/(float)frames;}

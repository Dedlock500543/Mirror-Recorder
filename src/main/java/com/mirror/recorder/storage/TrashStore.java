...
public class TrashStore{
    ...
    private void saveSessionTrash(){File sf=sessionStateFile();File tmp=new File(sf.getAbsolutePath()+".tmp");try(BufferedWriter w=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(tmp),"UTF-8"))){for(String name:sessionTrash){w.write(name);w.newLine();}}catch(Exception e){LOG.error("Failed to save session trash state",e);tmp.delete();return;}if(sf.exists()&&!sf.delete()){LOG.error("Failed to replace session trash state");tmp.delete();return;}if(!tmp.renameTo(sf)){LOG.error("Failed to rename session trash tmp file");tmp.delete();}}
